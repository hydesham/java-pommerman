...
public class SingleTreeNodeAF {
    public MCTSParamsAF params;

    private SingleTreeNodeAF parent;
    private SingleTreeNodeAF[] children;
    ...
    SingleTreeNodeAF(MCTSParamsAF p, Random rnd, int num_actions, Types.ACTIONS[] actions) {
        this(p, null, -1, rnd, num_actions, actions, 0, null);
    }

    private SingleTreeNodeAF(MCTSParamsAF p, SingleTreeNodeAF parent, int childIdx, Random rnd, int num_actions,
        ...
        children = new SingleTreeNodeAF[num_actions];
        totValue = 0.0;
    ...
    void mctsSearch(ElapsedCpuTimer elapsedTimer) {
        ...
        while(!stop){
            ...
            SingleTreeNodeAF selected = treePolicy(state);
    ...
    private SingleTreeNodeAF treePolicy(GameState state) {

        SingleTreeNodeAF cur = this;
    ...
    private SingleTreeNodeAF expand(GameState state) {
        ...
        SingleTreeNodeAF tn = new SingleTreeNodeAF(params,this,bestAction,this.m_rnd,num_actions,
    ...
    private SingleTreeNodeAF uct(GameState state) {
        SingleTreeNodeAF selected = null;
        double bestValue = -Double.MAX_VALUE;
        for (SingleTreeNodeAF child : this.children)
        {
    ...
    private void backUp(SingleTreeNodeAF node, double result)
    {
        SingleTreeNodeAF n = node;
        while(n != null)
        {
            // double decayedReward = decayReward(result, n.m_depth);
            n.nVisits++;
            n.totValue += result;
    ...
    private boolean notFullyExpanded() {
        for (SingleTreeNodeAF tn : children) {
            if (tn == null) {
                return true;
            }
        }
    ...
    private double decayReward(double reward, int depth) {
        // Addition - Backpropagation MCTS Enhancement.
        double gamma = 0.95;
